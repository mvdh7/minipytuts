...
flask = pd.read_table(
    filename,
    sep="\s+",  # regular expression / regexp
    skiprows=68,
    names=headers,
)

# Get datetime
# dt_cols = ["sample_year", "sample_month", ] #etc
# dt_cols = {"sample_year": "year", "sample_month": "month"}
dt_cols = {"sample_" + t: t
...
flask["datetime"] = pd.to_datetime(
    flask[dt_cols.keys()].rename(dt_cols, axis=1)
)
...
slope, intercept, r, p, se = linregress(
    flask[flask.xco2_good].datenum,
    flask[flask.xco2_good].xco2
)
...
ax.plot(fx, fy)
ax = axs[1]
sns.regplot(x="datenum", y="xco2", data=flask[::40], ax=ax,
            ci=99.9)
